...
# FILE = Path(__file__).resolve() # 得到当前文件的绝对位置
# ROOT = FILE.parents[0]  # YOLOv5 root directory得到当前文件的所在位置的上一级文件目录
# if str(ROOT) not in sys.path:
#     sys.path.append(str(ROOT))  # add ROOT to PATH
# ROOT = Path(os.path.relpath(ROOT, Path.cwd()))  # relative 子啊将ROOT变为相对路径
