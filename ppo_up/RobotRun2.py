...
class RobotRun2:
    ...
    def __init__(self, robot, state, action_leg_upper, action_leg_lower, action_ankle, step, zhua, gps0, gps1, gps2, gps3, gps4):
        ...
        self.LegUpper = 1.0 * self.action_leg_upper
        self.LegLower = 1.0 * self.action_leg_lower
        self.Ankle    = 1.0 * self.action_ankle
    ...
    def run(self):
        ...
        raw_11 = self.future_state[11]
        raw_13 = self.future_state[13]
        raw_15 = self.future_state[15]
        # 应用关节角度限制，确保动作在安全范围内
        ...
        if (raw_11 != self.future_state[11]) or (raw_13 != self.future_state[13]) or (raw_15 != self.future_state[15]):
            print(
                f"[CLAMP] step={self.step} "
                f"11:{raw_11:.3f}->{self.future_state[11]:.3f} "
                f"13:{raw_13:.3f}->{self.future_state[13]:.3f} "
                f"15:{raw_15:.3f}->{self.future_state[15]:.3f}"
            )
        # 执行多个仿真步
        self.robot.step(32)
        self.robot.step(32)
        ...
        if self.touch1.getValue() == 1.0 or self.touch1_1.getValue() == 1.0 or self.touch1_2.getValue() == 1.0:
            ...
            if (self.touch_value[0] == 1 and self.touch_value[1] == 1 and self.touch_value[2] == 1 and total_distance > 0.03):
                reward = 50
                goal = 1
                count = 1
                done = 1
