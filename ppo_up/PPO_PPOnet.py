...
class ActorCritic(nn.Module):
    def __init__(self, act_dim, node_num):
        ...
        self.exploration_factor = 1.0  # 可调探索系数，训练早期更大以增加噪声
    ...
    def forward(self, x, state, x_graph):
        ...
        sigma = torch.exp(log_sigma) * self.exploration_factor
    ...
    def set_exploration_factor(self, factor):
        # 防止过低导致完全无探索
...
class PPO:
    ...
    def choose_action(self, episode_num, obs, x_graph, explore=None):
        ...
        # 动态调节探索强度：sigma_factor 会随训练逐渐减小
    ...
    def get_current_sigma(self):
        base_sigma = torch.exp(self.policy.actor_log_sigma).item()
        return base_sigma * getattr(self.policy, "exploration_factor", 1.0)

    def _get_exploration_factor(self, episode_num):
        """
        线性衰减探索强度：前期保留较大噪声，后期逐渐收敛
        """
        start_factor = 1.5
        end_factor = 0.2
        decay_episode = 4000
