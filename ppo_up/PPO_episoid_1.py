...
from python_scripts.PPO.HPPO_agent import HPPOAgent
...
def eval_success_rate_catch(
    ...
    """
    抓取任务评估：复用同一个 env，只 reset，不新建 Environment/Robot。
    """
    success = 0

    for ep in range(n_eval):
        env.reset()
        env.wait(settle_wait_ms)
        ...
        while True:
            ...
            action_shoulder, _, _, _ = ppo_shoulder.choose_action(
                episode_num=10**9, obs=obs, x_graph=robot_state, explore=False
            )
            action_arm, _, _, _ = ppo_arm.choose_action(
                episode_num=10**9, obs=obs, x_graph=robot_state, explore=False
            )

            upper_gate, _, _ = hppo_agent_upper.choose_action(
                group='upper', obs=obs, x_graph=robot_state, episode_num=10**9
            )
            gate_shoulder = float(upper_gate[0])
            gate_arm = float(upper_gate[1])
            ...
            if gate_shoulder < 0.5:
                action_shoulder_exec = last_action_shoulder
            else:
                action_shoulder_exec = action_shoulder_smooth
            if gate_arm < 0.5:
                action_arm_exec = last_action_arm
            else:
                action_arm_exec = action_arm_smooth
            last_action_shoulder = action_shoulder_exec
            last_action_arm = action_arm_exec

            gps1, gps2, gps3, gps4, _ = env.print_gps()
            catch_flag = 1.0 if steps >= 19 else 0.0

            next_state, _, done, _, _, _ = env.step(
                robot_state,
                action_shoulder_exec,
                action_arm_exec,
                steps,
                catch_flag,
                gps1, gps2, gps3, gps4,
            ...
            left_any = any(all_grasp[0:3])
            right_any = any(all_grasp[3:6])
    ...
    return 100.0 * success / max(n_eval, 1)
...
def PPO_episoid_1(model_path=None, max_steps_per_episode=500):   
    ...
    # 创建两个独立的HPPO智能体：一个控制上半身（2个离散值），一个控制下半身（3个离散值）
    hppo_agent_upper = HPPOAgent(node_num=19, action_splits={'upper': 2})  # 上半身智能体
    hppo_agent_lower = HPPOAgent(node_num=19, action_splits={'lower': 3})  # 下半身智能体
    ...
    for i in range(episode_start, episode_start + 10000):  # 从episode_start开始，最多再训练50000个周期
        ...
        current_shoulder_value = robot_state[0] if len(robot_state) > 0 else 0.0
        current_arm_value = robot_state[1] if len(robot_state) > 1 else 0.0
        last_action_shoulder = current_shoulder_value
        last_action_arm = current_arm_value
        ...
        while True:
            ...
            upper_gate, hppo_log_prob_upper, hppo_value_upper = hppo_agent_upper.choose_action(
                group='upper',
                obs=obs,
                x_graph=robot_state,
                episode_num=i
            ...
            print(f'第{i}周期，第{steps}步，肩膀动作: {action_shoulder_smooth:.4f}，手臂动作: {action_arm_smooth:.4f}，离散动作: {upper_gate[0],upper_gate[1]},实际动作: {action_shoulder_exec:.4f}, {action_arm_exec:.4f}')
            # 更新上一动作（用于下一次平滑）
            prev_action_shoulder = action_shoulder_smooth
            prev_action_arm = action_arm_smooth
            ...
            # 执行一步动作（使用平滑后的动作）
            next_state, reward_env, done, good, goal, count = env.step(robot_state, action_shoulder_exec, action_arm_exec, steps, catch_flag, gps1, gps2, gps3, gps4, img_name)
            ...
            # 密集奖励：当靠近目标时，给予一个与距离成反比的奖励
            if current_distance < 0.1: # 10cm范围内
                reward += (0.1 - current_distance) * 20 # 距离越近，奖励越高

            if success_flag1 == 1:                       # 抓到了
                if current_distance <= 0.1:             # 10 cm 容忍
                    reward += 100
                    print("✅ 抓到目标梯级，发放大奖励！")
                else:
                    reward += 30                         # 抓错梯子，惩罚降低
                    print("⚠️  抓到非目标梯级，给小奖励")

            if done == 1 and success_flag1 != 1:
                if steps > 2: # 忽略不稳定的早期结束
                    print("回合结束但未抓取，给予惩罚！")
                    reward -= 10 # 惩罚降低
            ...
            if gate_shoulder < 0.5 and gate_arm < 0.5:
                reward -= 5.0  # 惩罚值可调
                print(f"⚠️  警告：所有上半身离散动作都为0，给予惩罚 -5.0")
            ...
            if should_store:
                ...
                hppo_agent_upper.store_transition(
                    group='upper',
                    ...
                    value=hppo_value_upper,
                    log_prob=hppo_log_prob_upper
                )
            else:
                # 不存储此样本（用于调试，可以打印信息）
            ...
            if len(robot_state) > 0:
                current_shoulder_value = robot_state[0]
            if len(robot_state) > 1:
                current_arm_value = robot_state[1]
            ...
            if done == 1 or steps >= max_steps_per_episode:
                ...
                loss_hppo_upper = hppo_agent_upper.learn('upper')
                loss = loss_shoulder + loss_arm + loss_hppo_upper  
                print('loss_arm:', loss_arm)
                print('loss_shoulder:', loss_shoulder)
                print('loss_hppo_upper:', loss_hppo_upper)
                print('loss:', loss)
                log_writer_catch.add(loss=loss)
                log_writer_catch.add(loss_arm=loss_arm)
                log_writer_catch.add(loss_shoulder=loss_shoulder)
                log_writer_catch.add(loss_hppo_upper=loss_hppo_upper)
                ...
                base_checkpoint_data = {
                    ...
                    'policy_hppo_upper': hppo_agent_upper.policy.state_dict(),
                    'optimizer_hppo_upper': hppo_agent_upper.optimizer.state_dict(),
                    'episode': i
                }

                # --- 【移除测试功能】定期保存模型，但不进行测试 ---
                if (i % CHECKPOINT_INTERVAL == 0) and (i != 0):
                    print(f"\n--- 周期 {i}: 到达检查点，开始评估成功率（用于只保留最优模型）---")

                    # ✅ 评估：复用同一个 env，只 reset，不允许新建 Environment/Robot
                    success_rate = eval_success_rate_catch(
                        env=env,
                        ppo_shoulder=ppo_shoulder,
                        ppo_arm=ppo_arm,
                        hppo_agent_upper=hppo_agent_upper,
                        n_eval=10,
                        max_steps=60,
                        settle_wait_ms=300
                    )

                    print(f"[EVAL] episode={i}, success_rate={success_rate:.2f}%")
                    log_writer_catch.add(success_rate=success_rate)
                ...
                log_writer_catch.add_log_prob_hppo_upper(hppo_log_prob_upper)
                log_writer_catch.add_value_hppo_upper(hppo_value_upper)
        ...
        if success_flag1 == 1:
            ...
            PPO_tai_episoid(ppo2_LegUpper=ppo2_LegUpper, ppo2_LegLower=ppo2_LegLower, ppo2_Ankle=ppo2_Ankle,
                            existing_env=env, total_episode=total_episode, episode=tai_episoid,
                            log_writer_tai=log_writer_tai, log_file_latest_tai=log_file_latest_tai,
                            hppo_agent=hppo_agent_lower)
            tai_episoid += 1 
